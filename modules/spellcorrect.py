...
def main():
    ...
    if args.lm:
        corrector.run(args.infile, args.outfile, args.lm)
    else:
        corrector.run(args.infile, args.outfile)

class SpellingCorrector(ModuleBase):

    name = 'SpellingCorrector'

    def run(self, infile, outfile, arg_lm=False, fix_keywords=True):
        d = enchant.DictWithPWL("en_CA", "dictionary.txt")
        mapping = {'labor': 'labour', 'laborer': 'labourer', 'color': 'colour', 'yeras': 'years', 'elergies': 'allergies', 'around12': 'around 12',
                   'learnt': 'learned', 'rigor': 'rigour', 'couldn': 'couldn\'t', 'didnt': 'didn\'t', 'didn': 'didn\'t', 'neighbor': 'neighbour', 'enjury': 'injury', 'h/o': 'h/o'}
