...
class SymptomTagger(ModuleBase):

    name = 'SymptomTagger'

    def run(self, infile, outfile, tagger="keyword_match", arg_sympfile=None, arg_chvfile=None):
        if tagger == "crf":
            self.crf_tagger(infile, outfile)
        elif tagger == "nn":
            self.seq2seq(infile, outfile, trainfile)
        else:
            tree = etree.parse(infile)
            if tagger == "keyword_match": # DO NOT USE KEYWORD MATCH FOR TEST/DEV
                tree = self.keyword_match(tree)
            elif tagger == "medttk":
                tree = self.medttk(tree)
            elif tagger == 'seq2seq':
                tree = self.seq2seq(tree)
            elif tagger == "tag_symptoms":
                tree = self.tag_symptoms(tree, arg_sympfile, arg_chvfile)
            #elif tagger == "textrank":
            #    tree = self.tag_keywords(tree)

            tree.write(outfile)
            data_util.fix_escaped_chars(outfile)

    def crf_tagger(self, infile, outfile):
    ...
    def seq2seq(self, infile, outfile, trainfile):
        trainfile = '/u/sjeblee/research/data/i2b2/all_timeml_fixed.xml'
        model_seq.run(trainfile, infile, outfile, 'nn')

    def keyword_match(self, tree, usecardinal=True):
        diff_threshold = 0.8
    ...
    def medttk(self, tree):
        text_infile = '/u/sjeblee/temp.txt' # TODO: fix the hard-coding!
    ...
    #
    #
    def tag_symptoms(self, tree, arg_sympfile, arg_chvfile):
        starttag = '<EVENT>'.encode('utf-8')
        endtag = '</EVENT>'.encode('utf-8')
        symptoms = []
        ...
        # Open SYMP.csv file
        if use_symptom1:
            print('reading symptoms from SYMP.csv')
        ...
        # Open tsv file.
        if use_symptom2:
            print('reading symptoms from CHV')
...
def get_substrings_with_limit(input_string, max_word_count):
    words = input_string.split(' ')
    length = len(words)
    subs = [words[i:j+1] for i in range(length) for j in range(i, min(length, i + max_word_count))]
    substrings = []
    for sub in subs:
        substrings.append(' '.join(sub))
    return substrings
...
def main():
    ...
    tagger.run(args.infile, args.outfile, args.tagger)
