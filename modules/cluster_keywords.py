...
class KeywordClusterer(ModuleBase):
    name = 'KeywordClusterer'
    clusterer = None

    def run(self, infile, outfile, clusterfile, vecfile, num_clusters=100, train=False):
        starttime = time.time()
        ...
        if train:
            ...
            self.clusterer = KMeans(n_clusters=num_clusters, max_iter=500)
            ...
            kw_clusters = self.clusterer.fit_predict(kw_vecs_np)
            ...
            cluster_centers = list(self.clusterer.cluster_centers_)
        ...
        # Test
            ...
            if ',' in infile:
                testfiles = infile.split(',')
                testoutfiles = outfile.split(',')
                ...
                testids, testkeywords, testvecs = read_xml_file(infile, vec_model, dim)
                test_clusters = classifier.predict(testvecs)
                testids_collapsed, clusters_collapsed = collapse(testids, test_clusters)
                test_emb, test_clusters_text = cluster_embeddings(clusters_collapsed, clusterfile, vecfile, return_phrases=True)
                write_clusters_to_xml(infile, outfile, testids_collapsed, clusters_collapsed, test_clusters_text)
