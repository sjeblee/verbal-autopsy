...
def create_lstm_model(X, Y, embedding_size, num_nodes, activation='sigmoid', dropout=0.1):
    Y = to_categorical(Y)
    X = numpy.asarray(X)
    print "train X shape: " + str(X.shape)
    ...
    lstm_out = LSTM(num_nodes, return_sequences=False)(attn_out)
...
def create_cnn_model(X, Y, embedding_size, act=None, window=3):
    ...
    for w in window_sizes:
        print "window: " + str(w)
        conv = Conv1D(max_seq_len, w, input_shape=input_shape)(attn_out)
