...
name="va_nb_test"   # Give the output folder a name, will be created in $prefix
modelname="nb"        # Model type: one of: nn cnn svm rnn gru
...
# DEV
python pipeline.py --name $name --model $modelname --dataset $dataset --modelname ${modelname}_${trainset}_${fname}_${labels} --train $trainset --dev $devset --preprocess $prep --features $feats --featurename $fname --labels $labels --vectors $vecfile --prefix $prefix --dataloc $dataloc | tee $prefix/data/$name/out-$devset-$modelname-$fname.txt
