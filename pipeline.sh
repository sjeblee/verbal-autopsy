...
# DEV
python pipeline.py --name $name --model $modelname --dataset $dataset --modelname ${name}_${trainset}_${fname} --train $trainset --dev $devset --preprocess "spell" --features $feats --featurename $fname --labels "ICD_cat_neo" | tee ../../data/$name/out-$devset-$modelname-$fname.txt
...
# CROSSVAL
#python pipeline.py --name $name --dataset $dataset --train $trainset --features $feats --featurename $fname --ex "crossval" --preprocess "spell" | tee ../../data/${name}/out-$devset-$fname.txt
