...
def main():
    ...
    if args.model == "rf":
        ...
        if args.train == "neonate" or args.train == "child_neo" or args.train == "child+neo":
            n_feats = 227
            nodes = 192
            anova = "chi2"
        else:
            n_feats = 398
    # Temp for PHMRC
    if args.train == "child+neo" or args.train == "child_neo":
        n_feats = 227
...
def crossval(arg_modelname, arg_train, arg_features, arg_featurename, arg_name, arg_preprocess, arg_labels, arg_dataset="mds_one"):
    ...
    for z in range(10):
        ...
        trainfile = datadir + "/train_" + trainname +  "_cat.xml"
        testfile = datadir + "/test_" + trainname + "_cat.xml"
...
def run(arg_model, arg_modelname, arg_train, arg_test, arg_features, arg_featurename, arg_name, arg_preprocess, arg_labels, arg_dev=True, arg_hyperopt=False, arg_dataset="mds_one", arg_n_feats=414, arg_anova="chi2", arg_nodes=297, dataloc="/u/sjeblee/research/va/data/datasets"):
    ...
    if not (os.path.exists(trainfeatures) and os.path.exists(devfeatures)):
        ...
        extract_features_phmrc.run(trainset, trainfeatures, devset, devfeatures, features, labels, stem, lemma)

    # Model
    if arg_hyperopt:
        print "Running hyperopt..."
        model_phmrc.hyperopt(modeltype, trainfeatures, devfeatures, devresults, resultsloc, labels)
    else:
        print "Creating model..."
        if modeltype == "nb":
            svm.run(modeltype, trainfeatures, devfeatures, devresults, labels)
        else:
            model_phmrc.run(modeltype, modelname, trainfeatures, devfeatures, devresults, resultsloc, labels, arg_n_feats, arg_anova, arg_nodes)
