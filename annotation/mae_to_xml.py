...
def main():
    ...
    if not (args.infile and args.ann_dir):
        print "usage: ./xml_to_mae_in.py --dir [path/to/dir] --in [file.xml] --out [file.xml]"
        exit()

    run(args.infile, args.outfile, args.ann_dir)

def run(arg_infile, arg_outfile, arg_dir):

    # TODO: get id from filename, store in dictionary
    ann_narrs = {} # map from record id to annotated narrative
    # for file in arg_dir:
    # TODO: save file contents in ann_narrs[id] = text

    # TODO: for each record in infile, find the annotated output, put in it in the narr_timeml_simple node
