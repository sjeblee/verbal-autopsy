...
def main():
    ...
    if train_mode== True:
        X,Y,label,emb_comb= preprocess(input_train)
...
def test(model,input_test, testfile,emb_dim_char=30,anova_filter=None, hybrid=False, rec_type=None, kw_cnn=None, threshold=0.01):
    ...
    testX = []
    testY = []
    testX,testY,testlabels,emb_dim_comb = preprocess(input_test)
...
def timeSince(since):
   ...
   s -= m * 60
   return '%dm %ds' % (m, s)
def preprocess(input_file,n_hidden=128,emb_dim_char=30,emb_dim_word = 100,max_num_word = 200,max_char_in_word = 7,learning_rate=0.001,vocab='abcdefghijklmnopqrstuvwxyz0123456789 '):
    global labelencoder
    labelencoder  = preprocessing.LabelEncoder()
