...
def main():
    ...
    csmf = []
    for i in range(10):
        input_train = "D:/projects/zhaodong/research/va/data/small_dataset/train_"+rec_type+"_"+str(i)+"_cat_spell.xml"  #input train file for char_embeeding
        input_test = "D:/projects/zhaodong/research/va/data/small_dataset/test_"+rec_type+"_"+str(i)+"_cat_spell.xml"    #input test file for char_embedding
        if train_mode== True:
            X,Y,label,emb_comb,ID= preprocess(input_train,i,emb_dim_char=emb_dim_char)
        ...
        testids, testlabels, predictedlabels = test(model,input_test,i,emb_dim_char=emb_dim_char)
...
def test(model,input_test,ind,emb_dim_char=24,anova_filter=None, hybrid=False, rec_type=None, kw_cnn=None, threshold=0.01):
    print("testing...")
    stime = time.time()
    testlabels = []
    testX = []
    testY = []
    testX,testY,testlabels,emb_dim_comb,testids = preprocess(input_test,ind,emb_dim_char=emb_dim_char)
...
def timeSince(since):
   ...
   s -= m * 60
   return '%dm %ds' % (m, s)
def preprocess(input_file,ind,n_hidden=128,emb_dim_char=24,emb_dim_word = 100,max_num_word = 200,max_char_in_word = 7,learning_rate=0.0001,vocab='abcdefghijklmnopqrstuvwxyz0123456789 '):
    global labelencoder
    labelencoder  = preprocessing.LabelEncoder()
