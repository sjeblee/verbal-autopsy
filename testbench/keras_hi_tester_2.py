...
def create_data_block(X, vocab_dict, emb_mat, embed_size=300):
	...
	for i in range(X.shape[0]):
		curr_row = X[i,:]
		data_slab = []
		curr_row_squeezed = np.squeeze(np.asarray(curr_row))
		for integer in curr_row_squeezed:
			if integer == 0:
				data_slab.append(np.zeros(embed_size))
			else:
				try:
					data_slab.append(np.squeeze(np.asarray(emb_mat[integer,:])))
					hits += 1
				except:
					data_slab.append(np.zeros(embed_size))
					misses += 1
		final_slab = np.matrix(data_slab)
		new_X.append(final_slab)

	return np.asanyarray(new_X)
...
X_train_base = np.matrix([np.array(data_combo[0]) for data_combo in train_data])
y_train = np.array([data_combo[1] for data_combo in train_data])
y_train_cat = to_categorical(y_train)

X_train = create_data_block(X_train_base, deva_index, embed_mat)

X_test_base = np.matrix([np.array(data_combo[0]) for data_combo in test_data])
y_test = np.array([data_combo[1] for data_combo in test_data])
y_test_cat = to_categorical(y_test)

X_test = create_data_block(X_test_base, deva_index, embed_mat)
...
model.fit(X_train, y_train_cat, validation_data=(X_test, y_test_cat), epochs=10, batch_size=16)
