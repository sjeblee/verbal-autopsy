...
def lineToTensor(narrative):
    tensor = torch.zeros([seq_length,1],device=cuda)
...
class CNN(nn.Module):
    def __init__(self,sequence_length,input_size, hidden_size, output_size, emb_size):
        ...
        self.conv9 = nn.Sequential(nn.Conv2d(1,3,kernel_size=(9,emb_size),stride=1),nn.ReLU())
        self.conv10 = nn.Sequential(nn.Conv2d(1,3,kernel_size=(10,emb_size),stride=1),nn.ReLU())
        self.conv11 = nn.Sequential(nn.Conv2d(1,3,kernel_size=(11,emb_size),stride=1),nn.ReLU())
        self.conv12 = nn.Sequential(nn.Conv2d(1,3,kernel_size=(12,emb_size),stride=1),nn.ReLU())
        if False:

            self.conv6 = nn.Sequential(nn.Conv2d(1,3,kernel_size=(6,emb_size),stride=1),nn.ReLU())
            self.conv7 = nn.Sequential(nn.Conv2d(1,3,kernel_size=(7,emb_size),stride=1),nn.ReLU())
            self.conv8 = nn.Sequential(nn.Conv2d(1,3,kernel_size=(8,emb_size),stride=1),nn.ReLU())
        self.linear = nn.Linear(3*num_conv,output_size)
...
#         print(output4.size())
        ...
        output9 = self.conv9(input.unsqueeze(0).unsqueeze(0))
        self.maxpool = nn.MaxPool1d(kernel_size=(self.seq_length-9+1))
        output9 = self.maxpool(output9.squeeze(3))
        
        output10 = self.conv10(input.unsqueeze(0).unsqueeze(0))
        self.maxpool = nn.MaxPool1d(kernel_size=(self.seq_length-10+1))
        output10 = self.maxpool(output10.squeeze(3))
        
        output11 = self.conv11(input.unsqueeze(0).unsqueeze(0))
        self.maxpool = nn.MaxPool1d(kernel_size=(self.seq_length-11+1))
        output11 = self.maxpool(output11.squeeze(3))
        
        output12 = self.conv12(input.unsqueeze(0).unsqueeze(0))
        self.maxpool = nn.MaxPool1d(kernel_size=(self.seq_length-12+1))
        output12 = self.maxpool(output12.squeeze(3))
        if False:
        ...
        final_output = torch.cat((output2,output3,output4,output5,output6,output7,output8,output9,output10,output11,output12),1)
        # print(final_output.size(),final_output2.size())
        # print(final_output.squeeze(2).size())
        # print(final_output2.squeeze(2).size())
