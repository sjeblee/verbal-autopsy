...
def main():
    ...
    if train_mode== True:
        ...
        Y = to_categorical(Y)
        model = model_lib_test.char_cnn_model_train(X,Y,learning_rate=learning_rate,batch_size=batch_size,num_epochs=num_epochs)
    ...
    for k,v in data.items():
        for i in range(len(v)):
            ...
            for j in range(len(v[i][1])):
                if j < max_num_char:
                    line.append(vocab.index(v[i][1][j]))
            line = to_categorical(numpy.array(line),num_classes=input_size)
            if line.shape[0] != max_num_char:
                zero_padding = numpy.zeros((max_num_char-line.shape[0],input_size))
