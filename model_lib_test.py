...
#          print(x.size(),3333333)
          ...
          if self.ensemble == False: # Train CNN with no ensemble  
              ...
              logit = x
          return logit
class CNN_Text_Elmo(nn.Module):

     def __init__(self, embed_dim, class_num,kernel_num=200, kernel_sizes=6, dropout=0.0, hidden_size = 100):
          super(CNN_Text_Elmo, self).__init__()
...
'''
def cnn_model(X, Y, batch_size=100, act=None, windows=[1,2,3,4,5], X2=[], num_epochs=10, learning_rate=0.001,loss_func='categorical_crossentropy',dropout=0.0, kernel_sizes=5):
    # Train the CNN, return the model
    st = time.time()
    Xarray = numpy.asarray(X).astype('float')
    Yarray = Y.astype('int') 
...
def elmo_model(X, Y, num_epochs=10, learning_rate=0.001,dropout=0.0, kernel_sizes=5):
    batch_size = 8
    # Train the CNN, return the model
    st = time.time()
    Xarray = numpy.asarray(X).astype('float')
    ...
    num_labels = Yarray.shape[-1]
    steps = 0
    cnn = CNN_Text_Elmo(dim, num_labels,dropout=dropout, kernel_sizes=kernel_sizes)
    cnn = cnn.cuda()
    ...
    for epoch in range(num_epochs):
        ...
        while True:
             ...
             batchY = Yiter[i:i+batch_size]
             character_ids = batchX.cuda().long()
