...
def main():
    ...
    stime = time.time()
    anova_filter = None
    input_train = "D:/projects/zhaodong/research/va/data/dataset/train_child_cat_spell.xml"
    input_test = "D:/projects/zhaodong/research/va/data/dataset/dev_child_cat.xml"
...
def test(model,input_test,arg_train_test,emb_dic,all_categories,max_num_char,emb_dim_char=30):
    ...
    for i in range(len(X)):
        ...
        line_tensor_word = line_tensor_word.unsqueeze(1)
        category = Y[i]
        category_tensor = torch.tensor(int(category), dtype=torch.long,device=cuda)
